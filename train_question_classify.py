...
def get_predict_class_and_prob(clf,tree_clf,X):
	y = clf.predict(X)
...
def test_model(test_data_path):
	...
	for line in open(test_data_path):
		...
		y,p=get_predict_class_and_prob(clf,tree_clf,X_test)
...
if __name__ == '__main__':
	...
	if os.path.exists('model/qcls.model'):
		...
		tree_clf=ExtraTreesClassifier(n_estimators=10)
		tree_clf.fit(X,Y)
		with open('model/t_qcls.model', 'wb') as fid:
			cPickle.dump(tree_clf, fid)
